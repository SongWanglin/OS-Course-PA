#include "defs.h"

int main(int argc, char** argv){
    FILE *fp;
    /* check if argv provided any file names */
    if (argc == 1){ 
        printf("my-zip: file1 [file2 ...]\n");
        exit(1);
    }
    for (int i=1; i<argc; i++){ //go through all *argv[i] except the first one (which is supposed to be ./my-zip)
        fp = fopen(*(argv+i), "r");

	/* check if this filename is valid */
        if (!fp){   
            printf("my-zip: cannot open file\n");
            exit(1);
        }
	RLE_encoding(fp, i, argc);
        fclose(fp);
    }
    return 0;
}


/* detailed implementations below */

void
RLE_encoding(FILE *fp, int i, int argc)
{
    char last = '\0', current = '\0';
    int count = 1;
        /* get the 1st character from file stream, return the corresponding ASCII or EOF*/
        if (last == '\0')
            last = getc(fp);

        /* 2-pointer tracking */
        while( (current = getc(fp))!= EOF) { // use "current" counter to search
            if(current==last){
                count++;    // count the same character
                continue;
            }
            // write to the output stream if we found the next different character, reset the counter to 1
            fwrite(&count, sizeof(int), 1, stdout);
            fwrite(&last, sizeof(char),  1, stdout);
            count = 1;
            last = current;
        }

        /* check if it is the last char in  the last file, if so, write what's still stored in the current tracker */
        /* (the last character of other files is already handled by the previous while loop) */
        if( i == argc-1){
            fwrite(&count, sizeof(int), 1, stdout);
            fwrite(&last, sizeof(char), 1, stdout);
            count = 1;
        }
}

