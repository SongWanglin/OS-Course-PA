...
int main(int argc, char* argv[]){
	...
	chunk_size = (1024*1024)*4;			// chunk_size * q_size should be greater than input file size
	...
	// mmap done, unleash the hounds, I mean, CPUs
...
void* produce(void *argv){
	...
	for(int i=0;i<N_files;i++){
		...
		N_chunks+=chunk_count;
        	...
        	chs = mmap(NULL, f_stat.st_size, PROT_READ, MAP_SHARED, f_desc, 0); 
		...
		for(int j=0;j<chunk_count;j++){
			...
			chs+= product.Size; 	// this chunk's done mapping, go to next chunk
			put(product);
		}
		close(f_desc);
	}
